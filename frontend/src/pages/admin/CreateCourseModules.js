...
const EditModules = () => {
    ...
    useEffect(() => {
        const fetchCourseAndModules = async () => {
            try {
                ...
                const modulesRes = await fetch(`/api/course-modules/${id}`);
                if (modulesRes.ok) {
                    const modulesData = await modulesRes.json();
                    if (modulesData.modules && modulesData.modules.length > 0) {
                        setModules(modulesData.modules.map(module => ({
                            id: module.id,
                            title: module.title || '',
                            duration: module.duration || '',
                            description: module.description || '',
                            video: null, // Don't pre-fill file object
                            videoTitle: module.video_title || '',
                            videoThumbnail: null, // Don't pre-fill file object
                        ...
                        // No existing modules, create empty one
                        setModules([createEmptyModule()]);
                    }
                } else {
                    // No modules found, create empty one
                    setModules([createEmptyModule()]);
                }
            } catch (err) {
  ...
  xhr.open('POST', 'http://localhost:5000/api/course-modules', true);
    ...
    if (xhr.readyState === 4) { // DONE
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try {
          ...
          alert(response.message || 'Failed to save modules');
        } catch {
                ...
                <main className="flex-grow p-6 bg-gray-100">
                    ...
                    {uploading && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 pointer-events-auto">
                            ...
                            <h2 className="text-lg font-semibold mb-4">Uploading, please wait...</h2>
                    ...
                    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
                        ...
                        {modules.map((module, index) => (
                            <div key={index} className="mb-8 p-4 border border-gray-200 rounded-lg relative">
                                ...
                                <div className="mb-4">
                                    ...
                                    {!module.video && !module.existingVideo ? (
                                        ...
                                        <div className="mt-1 flex items-center justify-between p-3 bg-indigo-50 rounded-md border border-indigo-200">
                                            <div className="flex items-center">
                        ...
                        <div className="flex justify-end pt-4 border-t border-gray-200">
                            ...
                            >
                                {editing ? 'Update' : 'Save'} All Modules
                            </button>
                        </div>
                    </form>
                </main>
